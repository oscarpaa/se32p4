...
(
    ...
    output logic [1:0] cmp_bits_o
);
    logic [1:0] cmp_bits;
    
    assign cmp_bits[0] = (oper_a_i == oper_b_i) ? 1'b1 : 1'b0;

    assign cmp_bits[1] = (oper_sign_i == 1'b0 && oper_a_i < oper_b_i) ? 1'b1 : 
                        (oper_sign_i == 1'b1 && $signed(oper_a_i) < $signed(oper_b_i)) ? 1'b1 : 1'b0;
    
    assign cmp_bits_o = cmp_bits;
    ...
    always_comb begin
        unique case (oper_type_i)
            ...
            ALUOP_SLT:  oper_res_o <= {31'b0, cmp_bits[1]};
            ALUOP_SLTU: oper_res_o <= {31'b0, cmp_bits[1]};
